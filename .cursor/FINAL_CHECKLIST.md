# FINAL CHECKLIST — Pre-Release Verification (Olympus Surge)
...
- [ ] `oracle/crypt/OracleCodec.kt` — `SEED_PHRASE` changed to a fresh
      opaque ASCII token
- [ ] `oracle/crypt/OracleCodec.kt` — `STREAM_LEN` changed to a new
      value in 16–48
- [ ] `oracle/crypt/OracleSecrets.kt` — six byte arrays freshly
      generated **after** the seed change
- [ ] `oracle/config/OracleFacade.kt` — `PACKAGE_ID`, `MARKET_ID`,
      `DISPLAY_NAME` all unique to this project
- [ ] `oracle/config/OracleLegal.kt` — three URLs unique to this
...
- [ ] `app/src/main/kotlin/**/*.kt` — package declaration + folder
      ...
      `:core`, `:engine`, `:game`)
- [ ] `oracle/gateway/OracleBeacon.kt` — `CHANNEL_ID` renamed;
...
- [ ] `res/drawable/ic_oracle_beacon.xml` — new vector, distinct
...
- [ ] Install the APK / AAB
- [ ] Open the app — **frame ONE shows `OracleOfflineScreen`**, not a
      splash, not a black screen, not the game
- [ ] Portrait background is `oracle_offline_portrait.webp`, landscape
      is `oracle_offline_landscape.webp`
...
- [ ] Enable Wi-Fi
- [ ] Tap **Retry** — app transitions into `OracleLoadingScreen`
- [ ] Loading bar starts at 0, grows monotonically, reaches 1.0 at
      the exact moment `OracleGateActivity` takes over — no freeze at
      100 %, no jump backwards, no restart
- [ ] "Loading…" caption cycles dots on a stable rhythm
- [ ] `OracleGateActivity` opens with the config URL — no game screen
      appears at any point (attribution is Non-organic)
- [ ] Kill the app, relaunch → `OracleGateActivity` reopens directly
...
### 5. Privacy policy
- [ ] Privacy Policy URL (`https://olymmpussurge.com/privacy-policy.html`)
...
- [ ] **Slot game (Zeus theme)** — UA ends with
      ...
      `appname/` suffix (see `oracle_user_agent.md` §2)
...
- [ ] Bottom of WebView reaches the bottom edge (no unnecessary
      inset)
- [ ] `OracleOfflineScreen` + `OracleInviteScreen`: buttons stay
...
- [ ] Notification uses the `ic_oracle_beacon` icon and the icon is
...
- [ ] With **Wi-Fi + mobile data OFF** on a fresh install where the
      ...
      (`SplashActivity` → `MainActivity` → `BattleActivity`)
...
- [ ] Config request logged with `[OracleGate] request body: …`
- [ ] Body is a **flat** JSON object
- [ ] Response logged with `[OracleGate] response: 200 …`
- [ ] On `{ok:true}` → `savedUrl` + `expires` written to
      `OracleVault`
- [ ] On `{ok:false}` first launch → `OracleMode.native` written
      **once**; no subsequent config request fires this install
- [ ] `expires` respected on returning launch
- [ ] Token refresh (`OracleTokenService.onNewToken`) triggers a
      fresh config POST
...
- [ ] Privacy Policy URL live before submission
      (`https://olymmpussurge.com/privacy-policy.html`)
- [ ] Support URL live (`https://olymmpussurge.com/support.html`)
