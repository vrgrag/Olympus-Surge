# Oracle Gray-Part Pitfalls — Native Kotlin Edition

Mistakes that bit during ports of the gray-flow template to native
Android Kotlin (Olympus Surge, Aug 2026). Each section lists the
...
-keep class com.olympussurge.olympussurgegame.oracle.gateway.OracleTokenService { *; }

# kotlinx.serialization polymorphism (used by OracleGate body)
-keepclassmembers class **$$serializer { *; }
-keep,includedescriptorclasses class com.olympussurge.olympussurgegame.oracle.model.** { *; }
```

---

## 2. VPN makes `OracleOfflineScreen` flash on a healthy connection

### Symptoms

- Switching VPN on/off briefly shows `OracleOfflineScreen` even though
  the network is up.
...
**A. Whitelist VPN as a real network:** in `OracleLink.hasAnyAdapter()`,
...
`OracleLink` splits the question in two: `hasAnyAdapter()` is the cheap
...
private fun onLost() {
    ...
    offlineJob.value = scope.launch {
        delay(700)
        _state.emit(OracleLinkState.Offline)
    }
}
private fun onAvailable() {
    offlineJob.value?.cancel()
    _state.emit(OracleLinkState.Online)
}
```

Do NOT debounce the very first synchronous boot check in
`OraclePortalActivity.onCreate()` — that must be instant.
...
before `OracleOfflineScreen` appears.

### Fix

In `WebViewClient.onReceivedError` (`OracleGateActivity`):
...
2. For known DNS / disconnect codes
   ...
   `OracleOfflineActivity`, passing the last settled URL as `resumeUrl`
...
After renaming `com.olympussurge...` → new package, R.java references
...
- NDK 27+ (libGDX brings its own natives — verify each ABI's `.so`
...
## 9. `OracleInviteScreen` — Skip button barely visible
...
For `OracleGateActivity` wrap the WebView in a container that consumes
...
Do NOT add the same inset to `OracleOfflineScreen` or
`OracleInviteScreen` — see §12.
...
`OracleOfflineScreen` and `OracleInviteScreen`:
...
Derive the position instead. `OracleArtAnchor.kt` replays the same
...
The card bounds in `OracleArtMetrics` are measured off the shipped
...
`res/drawable/ic_oracle_beacon.xml` must:
...
- `NotificationCompat.Builder.setColor(...)` for notifications the app
  builds itself in `OracleBeacon.render`.
...
**Preferred: promote the scheme.** `OracleUrlGuard.sanitize` rewrites
`http://` to `https://`, and every URL entering the flow — config
answer, push payload, deep link — passes through it, so one choke point
covers all of them. `OracleGateActivity.shouldOverrideUrlLoading` does
...
to the system. The hand-off lives in `OracleGateActivity`'s
...
`OracleUrlGuard.sanitize` before it can reach `loadUrl` at all.

---

## 16. `OracleOfflineScreen` — buttons oversized or misplaced
...
val card = if (landscape) OracleArtMetrics.OfflineLandscape
           else OracleArtMetrics.OfflinePortrait
...
`OracleGateActivity` overrides the back callback:
...
The launcher lives in `OracleGateActivity` rather than a helper class,
...
- [ ] ProGuard `-keep` rules for AppsFlyer + Firebase +
      `OracleTokenService` present
- [ ] `compileSdk = 36`, `targetSdk = 36`, `minSdk = 24` in
      `app/build.gradle.kts`
- [ ] `kotlin.incremental=false` in `gradle.properties` (Windows /
      paths with spaces)
- [ ] `OracleLink.hasAnyAdapter()` whitelist includes `TRANSPORT_VPN`
- [ ] `OracleLink.isReachable()` probes a raw IP over TCP, never a
...
- [ ] `OracleInviteScreen` Skip button rendered as a real gradient
      button (§9)
- [ ] All button labels use `lineHeight = fontSize` (§10)
- [ ] WebView handles cutout inset on all sides in landscape (§11)
- [ ] `OracleOfflineScreen` + `OracleInviteScreen` have NO safe-area
      inset — buttons stay centered on artwork (§12)
- [ ] Buttons anchored via `OracleArtMetrics`, and those bounds
      re-measured against the artwork currently shipping (§12)
- [ ] `ic_oracle_beacon.xml` is a branded vector, not the launcher
      icon (§13)
...
- [ ] Push URL is HTTPS-only, or cleartext whitelisted per domain
      (§15)
- [ ] `OracleOfflineScreen` buttons capped in width, orientation-aware
      positioning (§16)
- [ ] WebView back gesture returns one page inside, does not close the
      activity (§17)
- [ ] File chooser opens without a permission dialog (§18)
- [ ] `OracleRuntime` is the manifest `android:name`, and it calls
      `FirebaseApp.initializeApp` + `OracleAttribution.prime()` before
...
- [ ] A config request that times out leaves the mode `pending`;
      only an actual backend "no" (`OracleReply.answered`) commits
      `native`
- [ ] `OracleBeacon.extractUrl` reads both `EXTRA_URL` and the raw
...
- [ ] A `native` user never receives a tap URL and is never handed to
      the shell by a push
- [ ] `OracleRelay` delivers to a live shell instead of relaunching the
