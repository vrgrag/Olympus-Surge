# Oracle launch flow — routing, attribution, push

Read this before changing anything in `OraclePortalActivity`,
`OracleAttribution`, `OracleBeacon` or `OracleRelay`. Every rule below
...
`OracleVault` (`olym_local_prefs` plain, `olym_sealed_prefs` encrypted).

| Key | Store | Meaning |
|---|---|---|
| `shell_pref_v1` | plain | `OracleMode`: `pending` / `web` / `native` |
...
```
no adapter          -> OracleOfflineActivity on frame one.
...
the life of the install because one socket timed out. `OracleReply`
...
`OracleGate.silence()` is the second case; `OracleGate.verdict()` is the
...
`prime()` belongs in `OracleRuntime` (the `Application`) and puts
...
1. **URL fails `OracleUrlGuard`** → dropped, notification shown as text.
...
3. **Shell alive** → `OracleRelay.offer()` hands it over in-process and
...
A data-only message reaches `OracleTokenService`, which builds the tap
...
extras instead. `OracleBeacon.extractUrl` therefore reads both our own
...
```
OracleAttribution  conversion arrived, af_status=Non-organic
OracleGate         response: 200 (N chars)
OraclePortal       -> shell
```
...
```
OracleAttribution  conversion arrived, af_status=Organic
OracleGate         response: 404
OraclePortal       backend ruled native
```

A launch that must **not** persist native:

```
OracleGate         request never landed / timeout
OraclePortal       endpoint unreachable, game for now, decision left open
```
