...
## Correct pattern (used in `OracleGateActivity.injectSafeAreaCss()`)
...
```javascript
// ✅ GOOD — inject inside OracleGateActivity via webView.evaluateJavascript(...)
(function () {
  if (window.__oracleSa) return; window.__oracleSa = true;
  var ID = '__oracle_sa';
  var CSS =
    ':root{' +
...
be true, and it is `installInsetRule()` in `OracleGateActivity` that
makes it so.
...
- ✅ Also skip the whole patch while the keyboard is open
     (compositor race — see `oracle_pitfalls.md` and the
     `kbOpen()` guard above).
...
- [ ] On landscape with a camera notch, no white bars on the sides
      (also see `oracle_pitfalls.md` §11 for the native cutout
      inset).
