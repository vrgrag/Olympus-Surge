# Oracle User-Agent — device authenticity + slot-vs-crash suffix
...
`OracleSecrets.kt` (via the packer script — see
`oracle_gray_guide.md` §"Packer script"). **Never ship two projects
...
The `OracleUaForge` class assembles this string from
...
opposite of what forging the UA is for. OracleUaForge.build() now
returns a plain Chrome-on-Android string and nothing else, and
OracleFacade.UA_APP_NAME has been deleted along with it.
...
- [ ] Log `OracleUaForge.value` in a debug run and confirm it matches
...
If any check fails, do not ship — fix `OracleUaForge.kt` first.
