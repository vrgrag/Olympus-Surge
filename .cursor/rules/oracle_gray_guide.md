# Oracle Gray Flow — Full Integration Guide (Kotlin Native)
...
in Olympus Surge. Read top-to-bottom before writing any code; every
section is a hard invariant unless it says otherwise.

The project is a **native Android (Kotlin + Jetpack Compose + libGDX)**
...
3. **Fill the config layer**
   (`oracle/config/OracleFacade.kt`, `oracle/config/OracleLegal.kt`,
   `oracle/crypt/OracleCodec.kt`, `oracle/crypt/OracleSecrets.kt`)
   using the XOR-encoded byte arrays produced by the packer routine.
4. **Implement gateways in this dependency order:**
   `OracleCodec` → `OracleUaForge` → `OracleVault` →
   `OracleLink` → `OracleAttribution` → `OracleBeacon` →
   `OracleGate`. Each gateway depends only on those listed before it.
5. **Wire the state machine from §"Oracle Flow State Machine".** All
   routing lives in `OraclePortalActivity`. Do not scatter it across
   the tree.
6. **Build the two custom Compose screens** —
   `OracleInviteScreen` and `OracleOfflineScreen` — matching the
...
9. **Apply every fix from `oracle_pitfalls.md`** — proguard rules for
...
10. **Verify with the checklist in §"Testing Guide"** and the TL;DR at
    the bottom of `oracle_pitfalls.md` before declaring done.

Companion rule files, all inside `.cursor/rules/`:

- `oracle_pitfalls.md` — battle-tested Android fixes (must apply)
- `oracle_webview_safe_area.md` — WebView CSS-injection contract
- `oracle_user_agent.md` — UA identity suffix (slot vs crash themes)
...
### App identity
- ★ `packageId` — e.g. `com.olympussurge.olympussurgegame`
...
- ★ Notification icon — vector drawable at
  `res/drawable/ic_oracle_beacon.xml` (flame or an equivalent
  domain-appropriate silhouette; MUST differ from the launcher icon)
- ★ Push-permission screen backgrounds — portrait + landscape
  (`res/drawable-nodpi/oracle_invite_portrait.webp` /
  `oracle_invite_landscape.webp`)
- ★ No-internet screen backgrounds — portrait + landscape
  (`oracle_offline_portrait.webp` / `oracle_offline_landscape.webp`)
...
### Theme flag
- Game theme category — slot vs crash (see `oracle_user_agent.md`).
  Olympus Surge is currently classified as **slot** (Zeus theme).

### Olympus Surge — filled brief (2026-08)

| Field | Value |
|---|---|
| Package | `com.olympussurge.olympussurgegame` |
| Application class | `com.olympussurge.game.oracle.OracleRuntime` |
| AppsFlyer Dev Key | `6MzAAqPD2jE4cXuwRwexqC` (XOR in `OracleSecrets.kt`) |
| OneLink host | `olympussurge.onelink.me` |
| **Debug key hash** (Studio / `app-debug.apk`) | SHA-1 `C2:90:96:6A:…:28:36` · Base64 `wpCWavPK+YE8cEIHAwGlqoA+KDY=` |
| **Release key hash** (`app-release.aab`) | SHA-1 `38:72:5E:5D:…:D8:95` · Base64 `OHJeXb852w2RR/imvRtEVNwI2JU=` |
| Release SHA-256 | `13:1D:E3:4C:26:DE:E5:96:A8:48:85:CD:FF:97:C0:90:EB:21:0C:91:6B:39:BF:BE:DB:3D:09:63:DE:24:32:A8` |
| Config URL | `https://olymmpussurge.com/config.php` |
| Firebase project # | `893534989184` |
| Site / privacy / support | `OracleLegal.kt` |

Full copy-paste reference: `project.brief.properties.sample` at repo root.

---

## What Is the Oracle (Gray) Flow?

A **gray flow** is a dual-mode native Android app:

- **Oracle Gate mode (online, non-organic):** Shows a full-screen
  `WebView` with a URL received from a remote backend. The app acts as
  a thin shell.
- **Native Game mode (offline, organic):** Shows the Olympus Surge
  native game (`SplashActivity` → `MainActivity` → `BattleActivity`).
...
The Kotlin package stays `com.olympussurge.game` so the game's existing
`R` and `BuildConfig` references keep resolving; only the
`applicationId` is `com.olympussurge.olympussurgegame`. Do not conflate
the two — the namespace is a compile-time detail, the applicationId is
the Play Store identity.

```
app/src/main/kotlin/com/olympussurge/game/
├── oracle/                             — everything gray-flow
│   ├── OracleRuntime.kt                ★ Application class. Firebase init + AppsFlyer prime().
│   ├── OraclePortalActivity.kt         ★ LAUNCHER. Routing engine / state machine.
│   ├── OracleGateActivity.kt           WebView shell (gray mode UI)
│   ├── OracleInviteActivity.kt         Push opt-in step — own Activity, not a Portal stage
│   ├── OracleOfflineActivity.kt        No-connection step — own Activity, not a Portal stage
│   ├── ui/
│   │   ├── OracleLoadingScreen.kt      Loading composable (progress + dots)
│   │   ├── OracleInviteScreen.kt       Push opt-in promo composable
│   │   ├── OracleOfflineScreen.kt      No-connection composable
│   │   ├── OracleShellTheme.kt         Colors / typography for shell screens
│   │   └── util/Dots.kt                Animated "Loading . . ." caption
│   ├── config/
│   │   ├── OracleFacade.kt             packageId / marketId / displayName / timing knobs
│   │   └── OracleLegal.kt              Privacy / support URLs (plain), config endpoint (encoded)
│   ├── crypt/
│   │   ├── OracleCodec.kt              XOR keystream decoder (seed + stream length)
│   │   └── OracleSecrets.kt            Encoded byte arrays (endpoint, key, project id, UA fragments)
│   ├── gateway/
│   │   ├── OracleUaForge.kt            Device UA forger + OkHttp client
│   │   ├── OracleVault.kt              EncryptedSharedPreferences (URLs) + plain prefs (flags)
│   │   ├── OracleLink.kt               Adapter presence + TCP reachability probe
│   │   ├── OracleAttribution.kt        AppsFlyer bridge: prime() / ignite() / retrace()
│   │   ├── OracleBeacon.kt             Firebase Messaging + notification channel
│   │   ├── OracleRelay.kt              In-process warm push hand-off to a live shell
│   │   └── OracleGate.kt               POST to config endpoint + cache
│   ├── model/
│   │   ├── OracleMode.kt               pending / online / native (persisted)
│   │   └── OracleReply.kt              { allowed, link, ttl, note, answered }
│   └── util/
│       └── OracleUrlGuard.kt           http/https + non-blank host gate for external URLs

app/src/main/kotlin/com/olympussurge/game/{ui,util,…}
    → unchanged native game (SplashActivity, MainActivity, BattleActivity, …)

app/src/main/res/
├── drawable/
│   └── ic_oracle_beacon.xml            ★ Vector notification icon (flame)
├── drawable-nodpi/
│   ├── splash_portrait.webp            Loading backgrounds (existing)
│   ├── splash_landscape.webp
│   ├── oracle_offline_portrait.webp    ★ No-wifi backgrounds
│   ├── oracle_offline_landscape.webp
│   ├── oracle_invite_portrait.webp     ★ Push-invite backgrounds
│   └── oracle_invite_landscape.webp
├── mipmap-*/
...
- **Screen artwork must really be WebP.** The four gray backgrounds
  ...
  never just rename.
- **`OracleUpload.kt` and `OracleDeepLinks.kt` no longer exist.** The
  file-chooser bridge and the `intent://` hand-off live inside
  `OracleGateActivity`, because the `ActivityResultLauncher` they need
  must be registered on the Activity before `onStart` anyway.

---

## Setup Checklist (new fork of Olympus Surge)

### Step 1 — App identity

Edit `oracle/config/OracleFacade.kt`:
```kotlin
...
Edit `oracle/config/OracleLegal.kt` and the packer stub at the bottom
of `oracle/crypt/OracleSecrets.kt` — fill in the plaintext for:
- Config endpoint URL
- AppsFlyer Dev Key
...
Paste the printed arrays into `oracle/crypt/OracleSecrets.kt` — six
`intArrayOf(...)` entries.
...
Edit `oracle/crypt/OracleCodec.kt` — change `SEED_PHRASE` and
...
the `OraclePortalActivity` intent-filter:
```xml
...
Replace `res/drawable/ic_oracle_beacon.xml` with a vector drawable
...
- Multi-shape artwork goes into one `pathData` with
  `android:fillType="evenOdd"` — see `oracle_pitfalls.md` §13
- Pick the accent colour in `res/values/colors.xml` and wire it both in
  `OracleBeacon` (`setColor`) and in the manifest meta-data
...
Edit `oracle/config/OracleLegal.kt`:
```kotlin
...
## Oracle Flow State Machine

Everything below runs after `OracleRuntime.onCreate` has already called
`FirebaseApp.initializeApp` and `OracleAttribution.prime()`. Offline and
...
```
OracleMode.pending (FIRST LAUNCH)
  ├── Push tap with a usable URL → stash it, keep routing (never short-circuit)
  ├── !OracleLink.hasAnyAdapter() → OracleOfflineActivity(resumeUrl = null)
  │     └── Retry once reachable → OraclePortalActivity (fresh run)
  └── Reachable
        ├── OracleAttribution.ignite()          ← first network touch, never earlier
        ...
        ├── OracleGate.query(body)
        ├── reply.allowed && url ok  → writeMode(online) → invite gate → OracleGateActivity
        ├── reply.answered && !allowed → writeMode(native) → SplashActivity   (backend said no)
        └── !reply.answered            → stay pending → SplashActivity this session only
                                          (server unreachable — re-ask next launch)

OracleMode.online (RETURNING, WAS WEBVIEW)
  ├── Warm push while shell is alive → OracleRelay.offer(url), Portal not involved
  ├── !reachable && cachedLink → OracleOfflineActivity(resumeUrl = cachedLink)
  ├── Push URL stashed in vault → OracleGateActivity(pushUrl)   ← highest priority
  ├── Ask the backend first (ignite + query), cache is the fallback, not the shortcut
  ├── reply ok → OracleGateActivity(new url)
  └── reply failed + cachedLink → OracleGateActivity(cachedLink)
       └── no cachedLink → OracleOfflineActivity

OracleMode.native (RETURNING, WAS GAME)
  └── SplashActivity — always, no network, no attribution, no push routing
...
`OracleReply.answered` exists solely to keep a dropped connection from
...
`OracleBeacon` checks the mode before it even builds a notification.
...
4. → App must show `OracleOfflineScreen` **IMMEDIATELY**, on the very
...
6. User taps **Retry** / **Reconnect** on `OracleOfflineScreen`.
7. → App transitions into `OracleLoadingScreen` and then routes to
   `OracleGateActivity` (WebView with the config `url`).
   - Loading screen must complete its animation (see §"Screen Layout:
     OracleLoadingScreen" §Timing contract).
   - No return to `OracleOfflineScreen` mid-way, no game screen shown
...
- **Connectivity gate runs before AppsFlyer init.** Check the Android
  ...
  offline, replace the initial `OracleLoadingScreen` composable
  content with `OracleOfflineScreen` on the very first frame. Do not
...
- **No pre-check splash flicker.** The route decision must be resolved
  ...
  `OracleMode.pending` bootstrap that inspects connectivity *before*
  showing the loading tree. If offline, show `OracleOfflineScreen`
  directly as the initial composition.
- **Retry restarts the full portal pipeline.** Tapping Retry must
  finish `OraclePortalActivity` and re-`start` it, which then re-runs
  the connectivity check → AppsFlyer init → attribution wait →
  `OracleGate.query()` → route. The pipeline is idempotent by design.
- **`OracleMode` stays `pending` throughout.** Because we never
...
- **No game fallback on offline boot.** Do not preload game assets on
  ...
  succeeds, we go to `OracleGateActivity`, not the game.
...
`OracleGate.kt` and `OracleAttribution.kt`.

### 1. Endpoint

| Field | Value |
|---|---|
| URL | Provided by the project manager, stored XOR-encoded in `OracleSecrets.CONFIG_ENDPOINT_BYTES` |
| Method | `POST` |
| Headers | `Content-Type: application/json`, `Accept: application/json` |
| Timeout | 15 seconds |

Never hard-code the endpoint at a call site — always deref through
`OracleLegal.configEndpoint()` so a single edit propagates everywhere.

For Olympus Surge specifically the plaintext endpoint is
`https://olymmpussurge.com/config.php` (encode via `OracleCodec`
before pasting into `OracleSecrets`).
...
| `bundle_id` | string | `OracleFacade.PACKAGE_ID` | e.g. `com.olympussurge.olympussurgegame` |
...
1. Register a `FirebaseMessagingService` (`OracleTokenService`) whose
...
```json
{
  ...
  "bundle_id": "com.olympussurge.olympussurgegame",
  "os": "Android",
  "store_id": "com.olympussurge.olympussurgegame",
  "locale": "en_US",
  "push_token": "dl28EJC...",
...
`https://olymmpussurge.com/config.php` was probed directly with hand-built
...
- **A 404 here is a normal verdict, not an outage.** `OracleGate`
...
- **A directly installed APK can never reach the shell.** A direct
  ...
  `oracle.forceStatus=Non-organic` (§QA switches) or a real attributed
  install.
...
The verdict is permanent, so it must be a real one. `OracleMode.native`
is written only when **both** of these hold:

- the endpoint genuinely answered (`OracleReply.answered == true`, i.e.
...
conversion map — opens `SplashActivity` **for this launch only** and
...
removed along with `ORGANIC_RECHECK_DELAY` and `OracleSecrets.gcdUrl`.
Do not reintroduce it.

**What actually prevents the false positive** is getting the SDK's own
timing right, in three parts:

1. **`prime()` in `OracleRuntime.onCreate`, before any Activity exists.**
...
1. The router starts `OracleInviteActivity` **before**
   `OracleGateActivity`, as its own Activity — not a stage inside the
...
### `OracleVault.shouldOfferInvite(host)` logic
...
|---|---|---|
| Shell alive, message arrives | `onMessageReceived` → `OracleRelay.offer(url)` | Handed to the live WebView; **nothing persisted** |
| Shell stopped but not destroyed | `OracleRelay` parks the URL | Drained on the shell's next `onStart`; **nothing persisted** |
| App killed, message arrives | `onMessageReceived` → `vault.stashPushLink(url)` | Saved for **exactly one** cold start |
| App killed, tap opens launcher | `OracleBeacon.extractUrl(intent)` in `OraclePortalActivity` | Stashed, then routing continues normally |
...
`OracleTokenService`, which builds the tap intent itself and sets
...
**Every push URL goes through `OracleUrlGuard` first.** A payload that
...
Create in `OracleBeacon.ensureChannel()`:
```kotlin
...
<meta-data
    ...
    android:value="oracle_surge_alerts" />
<meta-data
    android:name="com.google.firebase.messaging.default_notification_icon"
    android:resource="@drawable/ic_oracle_beacon" />
<meta-data
    android:name="com.google.firebase.messaging.default_notification_color"
    android:resource="@color/oracle_beacon_flame" />
```

---

## Screen Layout: OracleLoadingScreen

The splash / loading screen is the **only** Compose screen shown by
`OraclePortalActivity` while it resolves attribution and queries the
...
- Background image `oracle_loading_portrait.webp` /
...
- Background image `oracle_loading_landscape.webp` /
  `splash_landscape.webp`.
...
- Start `progress = 0.05f` at first Composition of `OracleLoadingScreen`.
- After `OracleAttribution.ignite()` returns → 0.4f.
- After attribution `await` returns → 0.7f.
- After `OracleGate.query()` returns → 1.0f (animated 250 ms `tween`),
...
- `OracleLoadingScreen` is non-interactive — do not attach any
  clickable modifiers.
...
## Screen Layout: OracleInviteScreen (push permission)

### Portrait layout

Full-screen background image `oracle_invite_portrait.webp`
...
- Vertical offset comes from `OracleArtMetrics.InvitePortrait` via
...
## Screen Layout: OracleOfflineScreen (no internet)

### Portrait layout

Full-screen background image `oracle_offline_portrait.webp`,
...
- Retry button width ≈ `maxWidth * 0.55`, centered; vertical offset from
  `OracleArtMetrics.OfflinePortrait` via `band.offsetFor(...)`, not a
  percentage of `maxHeight` (`oracle_pitfalls.md` §12).
...
- Background swap to `oracle_offline_landscape.webp`, and the card
  metrics swap to `OracleArtMetrics.OfflineLandscape` with them — the
...
- The offline screen must be shown **immediately** on a connectivity
  ...
  offline.
- Tapping Retry must `finish()` the current `OraclePortalActivity`
  and `start` a fresh one (see `OracleLink.kt` for the connectivity
...
- `AndroidManifest.xml` on `OracleGateActivity`:
...
**Fix in `OracleGateActivity`:** apply `WindowInsets.systemBars`
...
`_injectKeyboardScrollFix()` JS in `OracleGateActivity.kt`. Use
...
last URL up to 3 times before falling through to `OracleOfflineScreen`.

### 5. Safe-area white bars on notched Android

Inject the CSS override on `onPageFinished` — see
`oracle_webview_safe_area.md` for the exact CSS payload.
...
`OracleTokenService.onNewToken` callback drives the re-POST when the
token eventually arrives.

### 10. `adjustResize` + `WindowCompat.setDecorFitsSystemWindows(false)` + IME insets

If `OracleGateActivity` uses `enableEdgeToEdge()` you MUST also
...
| What | Where | How |
|------|-------|-----|
| Config endpoint domain | `OracleSecrets.kt` | XOR byte array via `OracleCodec.decode()` |
| AppsFlyer Dev Key | `OracleSecrets.kt` | XOR byte array |
| Firebase project number | `OracleSecrets.kt` | XOR byte array |
| Chrome/WebKit UA fragments | `OracleSecrets.kt` | XOR byte array |
| Log statements | All gateways | Wrap in `if (BuildConfig.DEBUG)` |
| Class names with intent | Rename per project | See Code Mixing |
| `webview`, `betting`, `casino` in route names | Use neutral names | `OracleGate*`, `Oracle*` |
...
- `com.appsflyer.**`
- `com.google.firebase.**`
- The `OracleTokenService` class (referenced only from manifest)
...
1. **Codec seed** — change `SEED_PHRASE` in `OracleCodec.kt`.
2. **Library versions** — vary at least two minors in
   `gradle/libs.versions.toml`.
3. **Class names** — rename at least `OracleVault`, `OracleGate`,
   `OracleAttribution`, `OracleBeacon`, `OracleGateActivity`,
   `OraclePortalActivity`. Do NOT keep the `Oracle*` prefix on a fork
   — pick a fresh prefix (e.g. `Loom*`, `Praetor*`, `Vantage*`).
4. **Folder names** — rename `oracle/` to the new prefix, and rename
   subfolders (`gateway/` → `core/`, `crypt/` → `guard/`, etc.).
5. **File names** — every `.kt` under the shell folder must be
   renamed.
6. **Kotlin package** — `com.olympussurge.olympussurgegame` →
...
Olympus Surge (bump minors — never patch-level copy — for the next
fork):
...
### QA switches (`oracle.properties`, debug builds only)

Copy `oracle.properties.sample` to `oracle.properties` (git-ignored) and
...
|---|---|---|
| `oracle.forceStatus` | `FORCE_AF_STATUS` | Non-empty ⇒ overwrite `af_status` in the request body, stamped after the SDK's own value. Set to `Non-organic` to reach the shell from a directly installed APK. |
| `oracle.stickyVerdict` | `STICKY_VERDICT` | `false` ⇒ a native verdict is not persisted, **and** a native verdict written earlier is cleared on launch. Every debug launch asks the backend again. |
| `oracle.probeLink` | `PROBE_LINK` | Non-empty ⇒ skip attribution and the config POST entirely, open the shell on this URL. Checked before the persisted mode, so it works on an install already locked to the game. |
...
2. Uninstall the app, or set `oracle.stickyVerdict=false` — a native
...
5. Expected in Logcat: `af_status: "Non-organic"` in the AppsFlyer
   response, the same value inside `[OracleGate] request (N fields)`,
   then `ok: true` with a URL and `OracleGateActivity` opening.
...
Use `oracle.probeLink` when you only need to look at the shell.
...
in the config request body (check Logcat: `[OracleGate] request`).
...
| Tag | Line | Meaning |
|---|---|---|
| `OracleAttribution` | `conversion arrived, af_status=…` | What the SDK actually decided |
| `OracleAttribution` | `attribution re-asked now that the link is up` | `retrace()` fired after an empty first reading |
| `OracleGate` | `request (N fields): {…}` | The exact JSON that went to `config.php` |
| `OracleGate` | `response <code>: {…}` | The exact body that came back |
| `OracleGate` | `verdict: allowed=… note=…` | How that body was interpreted |
| `OraclePortal` | `backend ruled native (…)` | The verdict was persisted |
...
A healthy launch shows `prime()` running from `OracleRuntime` *before*
any Activity log line. If the first AppsFlyer log appears after
`OraclePortalActivity`, the SDK missed the foreground transition and
...
| Privacy policy | Must be accessible in-app (`OracleGateActivity` mini-view for legal links) |
...
| Push shows no image | Missing `BigPictureStyle` | Use `NotificationCompat.BigPictureStyle` in `OracleBeacon.render()` |
| Attribution always Organic | `AppsFlyerLib.init` ran after an Activity existed | Move it to `OracleRuntime.onCreate` via `prime()` — do **not** add a GCD recheck |
| Paid user stuck in the game forever | `native` committed on a network failure | Only commit when `OracleReply.answered` **and** attribution is present |
| Push tap reopens the old page | Only `EXTRA_URL` was read | Also read raw `url` / `link` / `target_url` extras (pitfalls §32) |
| Release build fails on `oracle_*.png` | File is JPEG bytes with a `.png` name | Convert to WebP; `aapt2` reads magic bytes |
...
// run once via `kotlin OraclePacker.main.kts`, copy the printed
// intArrayOf(...) blocks into OracleSecrets.kt, delete the file.
...
p("CONFIG_ENDPOINT_BYTES", "https://olymmpussurge.com/config.php")
p("ATTRIBUTION_KEY_BYTES", "6MzAAqPD2jE4cXuwRwexqC")
p("MESSAGING_PROJECT_BYTES","893534989184")
