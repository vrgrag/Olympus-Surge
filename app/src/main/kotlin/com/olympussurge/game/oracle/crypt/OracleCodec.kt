...
/**
 * XOR keystream string hider used by [OracleSecrets] to keep the config
 ...
 *
 * The transform is symmetric — the same routine both encodes (in
 * `tools/pack_secrets.py`) and decodes (in [decode]).
 *
 * ─────────────────────────────────────────────────────────────
 ...
 *   2. Paste the fresh byte arrays into [OracleSecrets].
 *   3. Old arrays no longer decode — delete stale ones.
 */
object OracleCodec {

    // [FINGERPRINT] Change per project.
    private const val SEED_PHRASE = "OlymFlow_p8x!v27"

    // [FINGERPRINT] Change per project (16..48).
    private const val STREAM_LEN = 27
