 ...
 *
 * Nothing here touches the network. [OracleAttribution.prime] only
 * registers callbacks; the call that actually speaks to AppsFlyer is
 * [OracleAttribution.ignite], and the router fires it only once it has
 * confirmed a live connection.
 */
class OracleRuntime : Application() {

    /** Shared across the router, the shell and the messaging service. */
    lateinit var tracker: OracleAttribution
        private set
    ...
    override fun onCreate() {
        ...
        tracker = OracleAttribution(this)
        tracker.prime()
    }
