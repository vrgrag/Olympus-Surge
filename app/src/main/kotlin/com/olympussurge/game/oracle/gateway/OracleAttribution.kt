 ...
 * [com.olympussurge.game.oracle.OracleRuntime]. It puts nothing on the
 ...
 */
class OracleAttribution(private val ctx: Context) {
    ...
    fun prime() {
        ...
        val key = OracleFacade.attributionKey
        if (key.isEmpty()) {
    ...
    fun ignite(host: Activity) {
        prime()
        if (OracleFacade.attributionKey.isEmpty()) return
    ...
    suspend fun collectBody(firstLaunch: Boolean): JSONObject = coroutineScope {
        val budget = when {
            retraced -> OracleFacade.ATTRIBUTION_TIMEOUT_MS_RETRACE
            firstLaunch -> OracleFacade.ATTRIBUTION_TIMEOUT_MS
            else -> OracleFacade.ATTRIBUTION_TIMEOUT_MS_RESUME
        }

        val linkWait = async {
            withTimeoutOrNull(OracleFacade.DEEP_LINK_TIMEOUT_MS) { deepLink.await() }
        }
        val dataWait = async {
    ...
    private companion object {
        const val TAG = "OracleAttribution"
    }
}
