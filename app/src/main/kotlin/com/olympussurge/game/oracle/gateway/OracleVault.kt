 ...
 */
class OracleVault(context: Context) {
    ...
    fun readMode(): OracleMode = OracleMode.fromWire(plain.getString(K_MODE, null))

    fun writeMode(mode: OracleMode) {
    ...
    fun armInviteCooldown() {
        plain.edit()
            .putLong(K_INVITE_UNTIL, nowSeconds() + OracleFacade.INVITE_COOLDOWN_SECONDS)
            .apply()
    }
    ...
    /** Below API 33 there is no permission to hold. */
    ...
    private companion object {
        // Neutral filenames — do NOT include "oracle", "webview" or
        // partner domain names in these strings.
        const val PLAIN_FILE = "olym_local_prefs"
        const val SECRET_FILE = "olym_sealed_prefs"

        const val K_MODE = "shell_pref_v1"
        const val K_CACHED_LINK = "cache_blob"
        const val K_LINK_TTL = "cache_ttl"
        const val K_PUSH_LINK = "pending_blob"
        const val K_PUSH_TOKEN = "beam_id"
        const val K_INVITE_UNTIL = "invite_after"
        const val K_PUSH_ALLOWED = "beam_ok"
        const val K_PUSH_BLOCKED_OS = "beam_blocked"
        const val K_PUSH_OS_ASKED = "beam_asked"
    }
}
