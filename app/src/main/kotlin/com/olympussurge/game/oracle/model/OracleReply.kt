...
/**
 * Parsed reply from [com.olympussurge.game.oracle.gateway.OracleGate].
 *
...
data class OracleReply(
    ...
    companion object {
        ...
        fun parse(body: String): OracleReply = try {
            val raw = json.decodeFromString(OracleRawReply.serializer(), body)
        ...
        fun verdict(note: String): OracleReply =
            OracleReply(allowed = false, note = note, answered = true)

        /** Nothing was reached — no verdict, nothing to persist. */
        fun silence(note: String): OracleReply =
            OracleReply(allowed = false, note = note, answered = false)
    }
}
